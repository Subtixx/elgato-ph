...
function BrightnessPlusAction(inContext, inSettings) {
    ...
    this.onKeyUp = function(inContext, inSettings, inCoordinates, inUserDesiredState, inState) {
		...
		// Convert brightness
		var brightness = parseInt(objCache.brightness)+parseInt(inSettings.brightnessAdd);
		if (brightness > 254) {
			brightness = 254;
			showAlert(inContext);
    ...
    function setDefaults() {
        ...
        if ('brightnessAdd' in settings) {
            return;
        }

        // Set the brightness to 1
        settings.brightnessAdd = 1;

        // Save the settings
        saveSettings('com.subtixx.philips-hue.brightnessplus', context, settings);
    }
}
