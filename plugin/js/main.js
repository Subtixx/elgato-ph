        ...
        Object.keys(actions).forEach(function(inContext) {
            ...
            actions[inContext].newCacheAvailable(function() {
                ...
                if (actions[inContext] instanceof PowerAction) {
                    action = 'com.subtixx.philips-hue.power';
                }
                else if (actions[inContext] instanceof ColorAction) {
                    action = 'com.subtixx.philips-hue.color';
                }
                else if (actions[inContext] instanceof CycleAction) {
                    action = 'com.subtixx.philips-hue.cycle';
                }
                else if (actions[inContext] instanceof BrightnessAction) {
                    action = 'com.subtixx.philips-hue.brightness';
                }
                else if (actions[inContext] instanceof BrightnessPlusAction) {
                    action = 'com.subtixx.philips-hue.brightnessplus';
                }
                else if (actions[inContext] instanceof BrightnessMinusAction) {
                    action = 'com.subtixx.philips-hue.brightnessminus';
                }
                else if (actions[inContext] instanceof SceneAction) {
                    action = 'com.subtixx.philips-hue.scene';
                }
    ...
    websocket.onmessage = function(inEvent) {
        ...
        if(event === 'keyUp') {
            ...
            if (!(context in actions)) {
                ...
                if(action === 'com.subtixx.philips-hue.power') {
                    actions[context] = new PowerAction(context, settings);
                }
                else if(action === 'com.subtixx.philips-hue.color') {
                    actions[context] = new ColorAction(context, settings);
                }
                else if(action === 'com.subtixx.philips-hue.cycle') {
                    actions[context] = new CycleAction(context, settings);
                }
                else if(action === 'com.subtixx.philips-hue.brightness') {
                    actions[context] = new BrightnessAction(context, settings);
                }
                else if(action === 'com.subtixx.philips-hue.brightnessplus') {
                    actions[context] = new BrightnessPlusAction(context, settings);
                }
                else if(action === 'com.subtixx.philips-hue.brightnessminus') {
                    actions[context] = new BrightnessMinusAction(context, settings);
                }
                else if(action === 'com.subtixx.philips-hue.scene') {
            ...
            if (piEvent === 'valueChanged') {
                ...
                if(action !== 'com.subtixx.philips-hue.power' && action !== 'com.subtixx.philips-hue.cycle') {
